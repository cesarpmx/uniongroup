...
function regresarInicio() {
    ...
    var homePanel = Ext.getCmp('start-panel');

    if (!homePanel) {
        Ext.require('Modulos.home', function () {
            var nuevoPanel = Ext.create('Modulos.home', {
                id: 'start-panel'
            });
            contentPanel.add(nuevoPanel);
            contentPanel.getLayout().setActiveItem(nuevoPanel);
            habilitar(); // si necesitas llamar esto
        });
    } else {
        contentPanel.getLayout().setActiveItem(homePanel);
        habilitar();
    }
}
...
function createStore(id,nom,bnd,qry,qry2/*,frm*/, autoLoad) {  
    autoLoad: Ext.isEmpty(autoLoad) ? false : autoLoad;
    var objCombo =Ext.create('Ext.data.Store', {
       ...
       // model: 'datosCombo',
        proxy: {                                                 
            type: 'ajax',
            url: contexto+'/ComboLoader?bnd='+bnd+'&qry='+qry+'&idCampo='+id+'&campo='+nom+'&campo2='+qry2,
            reader: {
                type: 'json',
...
function buscarProductos(bnd,empresa,estatus) {    
    var objCombo =Ext.create('Ext.data.Store', {
        autoDestroy: true,
        autoLoad: false,
        ...
        proxy: {                                                 
            type: 'ajax',
            url: contexto+'/ComboLoader?bnd='+bnd+'&empresa='+empresa+'&estatus='+estatus,
            reader: {
                type: 'json',
                successProperty: 'success',
                rootProperty: 'records'
...
function buscarIdPallet(bnd,almacen,estatus, producto) {    
    var objCombo =Ext.create('Ext.data.Store', {
        autoDestroy: true,
        autoLoad: true,
        ...
        proxy: {                                                 
            type: 'ajax',
            url: contexto+'/ComboLoader?bnd='+bnd+'&almacen='+almacen+'&estatus='+estatus+'&producto='+producto,
            reader: {
                type: 'json',
                successProperty: 'success',
                rootProperty: 'records'
...
function subirArchivo(id,nom,idfile,nomfile,bnd) {    
    var objSubir =Ext.create('Ext.data.Store', {
        autoDestroy: true,
        autoLoad: true,
       // model: 'datosCombo',
        proxy: {
            type: 'ajax',
            url: contexto+'/ComboLoader?bnd='+bnd+'&idCampo='+id+'&campo='+nom+'&idArchivo='+idfile+'&campo2='+nomfile,
            reader: {
                type: 'json',
                successProperty: 'success',
                rootProperty: 'records'
