...
// <editor-fold defaultstate="collapsed" desc="HttpServlet methods. Click on the + sign on the left to edit the code.">
    ...
    public String obtenUsuarioValido(HttpServletRequest request, HttpServletResponse response) {
        ...
        _error.message = "El usuario no existe o la informacion es incorrecta.";
        try {
            ...
            String service = props.getValueProp("Host") + props.getValueProp("ServiceLogin");
            String content = new ObjectMapper().writeValueAsString(usuario);
            String repuesta = requetPost.getPost(service, content, request);
            ...
            if (idUsuario != null) {
                Integer AuxIdUsuario = 0;
                Integer accesoUsuario=0;
                if (configUsuario.items != null) {
                    for (Item item : configUsuario.items) {
                        AuxIdUsuario = item.usuid;
                        accesoUsuario=item.umoacceso;
                    }
                }
                session.setAttribute("usuario", String.valueOf(AuxIdUsuario));
                ...
                session.setAttribute("almacen", configUsuario.almacenes);
                session.setAttribute("acceso", accesoUsuario);
                login.success = true;
                login.url = direccion;
...
//            _error.success = false;
//            _error.message = "Error Sistema. " + e.getMessage();
//            strJson = new ObjectMapper().writeValueAsString(_error);
//            e.printStackTrace();
        } finally {
    ...
    public void redireccionValido(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            //String direccion = "/jsp_general/homeView.jsp";
...
//            em.getTransaction().commit();
    ...
    private String obtenModulosUsuario(ConfigUsuario usuario) {
        ...
        //String welcome = "{xtype:'label',html:'Usuario: @usuario ', style:'color: rgb(45, 139, 255); font-weight: bold;'},";
        String welcome = "{xtype:'label',html:'Usuario: ', style:'color: white; font-weight: normal;'},{xtype:'label',html:'@usuario ', style:'color: rgb(91, 210, 255); font-weight: bold;'},";
            ...
            if (usuario.items != null) {
                ...
                for (Item item : usuario.items) {
                    if (item.modpadre == 0) {
                        ...
                        strModulos += "{text:'" + item.modnombre + "',accesoModulo:'" + item.umoacceso + "',nombreUsuario:'" + item.usunombre + "',iconCls:'" + item.modicono + "',scale:'small',handler:function(){LoadMenu(" + item.umomodid + ");}},";
                    }
                }
                strModulos += "]";
